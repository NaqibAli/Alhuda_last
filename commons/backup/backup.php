...
/**
 * Please Ask Abdisamad if you need any adetional Information
 * This file contains the Backup_Database class wich performs
 * a partial or complete backup of any given MySQL database
 * @author Abdisamad Ali 
 * @version 1.0
 */
...
define("BACKUP_DIR", 'C:\xampp\htdocs\alhuda\downloads\database/' . date("Y") . '/' . date("F")); // Comment this line to use same script's directory ('.')
...
define("BATCH_SIZE", 64000); // Batch size when selecting rows from database in order to not exhaust system memory
...
class Backup_Database
{
    ...
    public function __construct($host, $username, $passwd, $dbName, $charset = 'utf8')
    {
        ...
        $this->backupFile              = 'backup-' . $this->dbName . ' - ' . date("M d, Y - His", time()) . '.sql';
        $this->gzipBackupFile          = defined('GZIP_BACKUP_FILE') ? GZIP_BACKUP_FILE : true;
        $this->disableForeignKeyChecks = defined('DISABLE_FOREIGN_KEY_CHECKS') ? DISABLE_FOREIGN_KEY_CHECKS : true;
        $this->batchSize               = defined('BATCH_SIZE') ? BATCH_SIZE : 64000; // default 1000 rows
        $this->output                  = '';
    }
    ...
    public function backupTables($tables = '*')
    {
        try {
            ...
            if ($this->gzipBackupFile) {
                ...
                $this->obfPrint('New Backup File Name: '  . $this->backupFile, 1, 1);
            }
        } catch (Exception $e) {
            print_r($e->getMessage());
            return false;
...
<!-- Please Ask Abdisamad if you need any additional Information -->